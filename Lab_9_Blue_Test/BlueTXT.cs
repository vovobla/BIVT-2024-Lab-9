...
{
    [TestClass]
    public sealed class BlueTXT
    {
        private Random _rand = new Random();
        BlueSerializer _serializer = new BlueTXTSerializer();
        ...
        public void Test_01_Hierarchy()
        {
            ...
            Assert.IsTrue(_serializer is BlueTXTSerializer);
            Assert.IsFalse(_serializer is BlueJSONSerializer);
            Assert.IsFalse(_serializer is BlueXMLSerializer);
        }
        [TestMethod]
        public void Test_02_CreateFolder()
        {
            Assert.IsTrue(IFileManagerTest.Check_CreateFolder(_serializer, $"Blue_TXT"));
            CheckFolder();
        }
        [TestMethod]
        public void Test_03_CreateFile()
        {
            Assert.IsTrue(IFileManagerTest.Check_CreateFile(_serializer, $"Blue_TXT", $"Blue_task", "txt"));
            CheckFile();
        }
        [TestMethod]
        public void Test_04_Blue1_TXT()
        {
            Init();
            Test_03_CreateFile();
        ...
        private void Init()
        {
            _serializer = new BlueTXTSerializer();
        }
        private void CheckFolder()
        {
            var pathes = IFileManagerTest.Check_Properties(_serializer);
            Assert.AreEqual(pathes.folder, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_TXT"));
        ...
        private void CheckFile()
        {
            var pathes = IFileManagerTest.Check_Properties(_serializer);
            Assert.AreEqual(pathes.folder, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_TXT"));
            Assert.AreEqual(pathes.file, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_TXT", $"Blue_task.txt"));
        }
