...
{
    [TestClass]
    public sealed class BlueXML
    {
        private Random _rand = new Random();
        BlueSerializer _serializer = new BlueXMLSerializer();
        ...
        public void Test_01_Hierarchy()
        {
            ...
            Assert.IsTrue(_serializer is BlueXMLSerializer);
        }
        [TestMethod]
        public void Test_02_CreateFolder()
        {
            Assert.IsTrue(IFileManagerTest.Check_CreateFolder(_serializer, $"Blue_XML"));
            CheckFolder();
        }
        [TestMethod]
        public void Test_03_CreateFile()
        {
            Assert.IsTrue(IFileManagerTest.Check_CreateFile(_serializer, $"Blue_XML", $"Blue_task", "xml"));
            CheckFile();
        }
        [TestMethod]
        public void Test_04_Blue1_XML()
        {
            Init();
            Test_03_CreateFile();
        ...
        private void Init()
        {
            _serializer = new BlueXMLSerializer();
        }
        private void CheckFolder()
        {
            var pathes = IFileManagerTest.Check_Properties(_serializer);
            Assert.AreEqual(pathes.folder, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_XML"));
        ...
        private void CheckFile()
        {
            var pathes = IFileManagerTest.Check_Properties(_serializer);
            Assert.AreEqual(pathes.folder, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_XML"));
            Assert.AreEqual(pathes.file, Path.Combine(IFileManagerTest.GeneralPath, $"Blue_XML", $"Blue_task.xml"));
        }
